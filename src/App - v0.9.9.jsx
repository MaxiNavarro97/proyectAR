...
// NUEVO: Importaciones de react-pdf
...
import { 
  ...
  HelpCircle, Rocket, X, Sparkles,
  // NUEVO: Iconos para el modal de donación
  Coffee, HeartHandshake
} from 'lucide-react';

// --- CONSTANTES ---
const MESES = ["Ene", "Feb", "Mar", "Abr", "May", "Jun", "Jul", "Ago", "Sep", "Oct", "Nov", "Dic"];

// NUEVO: Fuente para el PDF
Font.register({
  family: 'Roboto',
...
// NUEVO: Componente Documento PDF
const MortgagePDFDocument = ({ data, summary }) => (
  <Document>
    <Page size="A4" style={pdfStyles.page}>
      <View style={pdfStyles.header}>
        <View>
          <Text style={pdfStyles.brandTitle}>ProyectAR</Text>
          <Text style={pdfStyles.brandSub}>Soberanía Financiera</Text>
        </View>
        <View>
           <Text style={{ fontSize: 8, color: '#64748b' }}>Reporte Generado: {new Date().toLocaleDateString('es-AR')}</Text>
        </View>
      </View>
      ...
      <View style={{ flexDirection: 'row', gap: 10, marginBottom: 15 }}>
          <View style={{ flex: 1, backgroundColor: '#eef2ff', padding: 8, borderRadius: 4 }}>
              <Text style={{ fontSize: 8, color: '#4f46e5', fontWeight: 'bold' }}>CUOTA INICIAL</Text>
              <Text style={{ fontSize: 12, fontWeight: 'bold' }}>{money(summary.cuotaInicial)}</Text>
          </View>
          <View style={{ flex: 1, backgroundColor: '#fff7ed', padding: 8, borderRadius: 4 }}>
              <Text style={{ fontSize: 8, color: '#ea580c', fontWeight: 'bold' }}>TOTAL INTERESES</Text>
              <Text style={{ fontSize: 12, fontWeight: 'bold' }}>{money(summary.totalIntereses)}</Text>
          </View>
          <View style={{ flex: 1, backgroundColor: '#f0f9ff', padding: 8, borderRadius: 4 }}>
              <Text style={{ fontSize: 8, color: '#0284c7', fontWeight: 'bold' }}>PAGO TOTAL EST.</Text>
              <Text style={{ fontSize: 12, fontWeight: 'bold' }}>{money(summary.totalPagadoFinal)}</Text>
          </View>
      </View>
...
// NUEVO: Modal de Nudge / Donación (Corregido el bug de descarga)
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-indigo-500/20 shadow-2xl w-full max-w-md overflow-hidden relative animate-in slide-in-from-bottom-10 sm:zoom-in-95 duration-300">
        <div className="p-6 sm:p-8 text-center relative overflow-hidden">
           ...
           <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 font-medium mb-6 leading-relaxed px-4">
             ProyectAR es una herramienta 100% gratuita y desarrollada a pulmón. Si te sirvió para tomar una mejor decisión, un cafecito ayuda a mantener los servidores y seguir sumando funciones.
           </p>
           <div className="space-y-3 mb-6">
              <a href="https://cafecito.app" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full py-3 bg-[#00cba9] hover:bg-[#00b899] text-white font-black rounded-xl uppercase tracking-widest text-xs transition-all shadow-md hover:shadow-lg hover:-translate-y-0.5">
           ...
           {/* ACÁ ESTABA EL ERROR: Le sacamos el onClick={onClose} a este div */}
           <div className="mt-2">
              {downloadLink}
           </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-indigo-500/20 shadow-2xl w-full max-w-lg overflow-hidden relative animate-in zoom-in-95 duration-300">
        ...
        <div className="p-8 text-left">
           <div className="flex items-center gap-2 mb-2">
             ...
             {/* MODIFICADO: Versión */}
             <h2 className="text-xl font-black uppercase text-slate-800 dark:text-white">Versión 1.1 (Fase 2)</h2>
           </div>
           ...
           <div className="space-y-4 mb-8">
              <div className="flex items-start gap-3">
                 <div className="p-2 bg-emerald-100 dark:bg-emerald-500/20 rounded-lg text-emerald-600 dark:text-emerald-400 shrink-0"><Activity className="w-4 h-4"/></div>
                 <div>
                    <h4 className="text-xs font-black uppercase dark:text-white">Motor REM Integrado</h4>
                    <p className="text-[10px] text-slate-400">Las proyecciones ahora se ajustan automáticamente con los datos oficiales del BCRA.</p>
                 </div>
              </div>
              <div className="flex items-start gap-3">
                 <div className="p-2 bg-sky-100 dark:bg-sky-500/20 rounded-lg text-sky-600 dark:text-sky-400 shrink-0"><ArrowRightLeft className="w-4 h-4"/></div>
                 <div>
                    <h4 className="text-xs font-black uppercase dark:text-white">Comparador de Alquileres</h4>
                    <p className="text-[10px] text-slate-400">Nuevo módulo para calcular costos totales de contratos de alquiler ajustados por ICL/IPC.</p>
                 </div>
              </div>
              {/* MODIFICADO: PDF Novedad */}
              <div className="flex items-start gap-3">
                 <div className="p-2 bg-indigo-100 dark:bg-indigo-500/20 rounded-lg text-indigo-600 dark:text-indigo-400 shrink-0"><Download className="w-4 h-4"/></div>
                 <div>
                    <h4 className="text-xs font-black uppercase dark:text-white">Nuevo: Reportes PDF Profesionales</h4>
                    <p className="text-[10px] text-slate-400">Descargá tu simulación con formato de informe, ideal para presentar en inmobiliarias o guardar.</p>
                 </div>
              </div>
           </div>

           <button onClick={onClose} className="w-full py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white font-black rounded-xl uppercase tracking-widest text-xs transition-all shadow-lg hover:shadow-indigo-500/25 active:scale-95 flex items-center justify-center gap-2 group">
      ...
      {hovered && (
        <div 
          ...
          style={{ 
            ...
            top: `${(hovered.y / h) * 100}%`,
            transform: hovered.x > (w * 0.7) ? 'translate(-100%, -110%)' : 'translate(-50%, -110%)'
          }}
        >
          ...
          <div className="space-y-1.5 text-[10px] md:text-[11px]">
            ...
            <div className="flex justify-between items-center gap-4 text-indigo-500 font-bold uppercase">
              <div className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-indigo-500" /><span className="uppercase">{isRent ? 'Alquiler' : 'Capital'}:</span></div>
            ...
            <div className="flex justify-between items-center gap-4 text-orange-500 font-bold uppercase">
              <div className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-orange-400" /><span className="uppercase">{isRent ? 'Expensas' : 'Interés'}:</span></div>
            ...
            <g key={i} onMouseEnter={() => setHovered({ x: x + barW / 2, y: h - padB - hInt - hPri - hGas, data: d })} onMouseLeave={() => setHovered(null)} className="group cursor-pointer">
              ...
              {(i % Math.ceil(sampled.length/10) === 0) && (
                <text 
                  ...
                  y={h - padB + 10} 
                  textAnchor="end" 
                  className="text-[10px] md:text-[11px] fill-slate-500 font-black uppercase tracking-tighter md:hidden"
                  transform={`rotate(-90, ${x + barW/2}, ${h - padB + 10})`}
                >
...
function MortgageCalculator({ uvaValue, remData, remStatus }) {
  ...
  // NUEVO: Estado para mostrar el modal de donación
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-full">
      ...
      {/* NUEVO: Modal de Donación con Link PDF incrustado */}
      {showDonationModal && (
        <DonationModal 
      ...
      <div className="lg:col-span-9 space-y-8 min-w-0">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl border dark:border-slate-800 shadow-sm overflow-hidden text-left text-[11px]">
          <div className="p-8 flex flex-col sm:flex-row justify-between items-center border-b dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50 gap-4">
            ...
            {/* NUEVO: Botones de exportación combinados */}
            <div className="flex w-full sm:w-auto gap-2">
              <button onClick={exportToCSV} className="flex-1 sm:flex-none px-6 py-4 bg-slate-200 dark:bg-slate-800 border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-white font-black rounded-xl shadow-sm hover:scale-105 transition-all uppercase tracking-widest leading-none">
                 <Download className="inline w-4 h-4 sm:mr-2" /> <span className="hidden sm:inline">CSV</span>
              </button>
              <button onClick={() => schedule.length > 0 && setShowDonationModal(true)} className="flex-[2] sm:flex-none px-6 py-4 bg-indigo-600 text-white font-black rounded-xl shadow-xl hover:scale-105 transition-all uppercase tracking-widest leading-none whitespace-nowrap">
                 <FileText className="inline w-4 h-4 sm:mr-2" /> <span className="hidden sm:inline">Descargar</span> PDF Pro
              </button>
            </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in slide-in-from-bottom-4 duration-500 max-w-full overflow-hidden">
      <div className="lg:col-span-3 bg-white dark:bg-slate-900 p-8 rounded-3xl shadow-xl border dark:border-slate-800 space-y-8 text-left">
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border dark:border-slate-800">
            <span className="text-[10px] font-black uppercase text-slate-500 leading-tight">¿Ajustar Expensas por inflación? (Mensualmente)</span>
        ...
        <div>
          <label className="text-[10px] font-black uppercase text-slate-400 mb-4 block leading-none">DURACIÓN DEL CONTRATO</label>
        ...
        <div>
          <label className="text-[10px] font-black uppercase text-slate-400 mb-4 block leading-none">RECURRENCIA AJUSTE (MESES)</label>
          <div className="grid grid-cols-3 gap-2">
            {[3, 4, 6].map(m => (
        ...
        <div className="pt-6 border-t dark:border-slate-800">
          ...
          {inflationMode === 'rem' && (
            <p className="text-[9px] text-slate-400 mt-2 mb-4 px-1 leading-tight">Nota: El REM proyecta 36 meses, cubriendo este contrato.</p>
          )}
          {inflationMode === 'manual' && (
            <div className="animate-in fade-in space-y-2">
               <div className="flex justify-between items-center leading-none"><span className="text-[10px] font-black text-emerald-500 uppercase leading-none">Tasa fija anual estimada</span><span className="text-[11px] font-mono font-black dark:text-white leading-none">{manualInf}%</span></div>
      ...
      <div className="lg:col-span-9 space-y-8">
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl border dark:border-slate-800 shadow-sm relative overflow-hidden text-left">
          ...
          <div className="h-[280px] md:h-[450px] w-full">
...
// REEMPLAZAR CON TU ID DE MEDICIÓN DE GOOGLE ANALYTICS CUANDO LO TENGAS
...
export default function App() {
  ...
  useEffect(() => {
    // 1. Google Analytics (¡Sin la trampa!)
    if (GA_MEASUREMENT_ID) {
       ReactGA.initialize(GA_MEASUREMENT_ID);
       ReactGA.send({ hitType: "pageview", page: window.location.pathname });
    }

    // NUEVO: 2. Lógica del Pop-up con nueva key para que vuelva a salir en la v1.1
    const hasSeenWelcome = localStorage.getItem('proyectar_welcome_beta_v2');
    if (!hasSeenWelcome) {
      setShowWelcome(true);
    }
  }, []);
  ...
  const handleCloseWelcome = () => {
    localStorage.setItem('proyectar_welcome_beta_v2', 'true');
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 font-sans transition-colors flex flex-col max-w-[100vw] overflow-x-hidden relative">
        ...
        <nav className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-3xl border-b dark:border-slate-800 sticky top-0 z-40 min-h-[80px] h-auto md:h-28 flex flex-col md:flex-row items-center justify-between px-4 md:px-10 py-4 md:py-0 gap-4 md:gap-0 shadow-sm leading-none">
          ...
          <div className="flex items-center gap-3 md:gap-5">
            <div className="bg-indigo-600 text-white p-2.5 md:p-4 rounded-2xl shadow-xl shadow-indigo-600/30">
              <Calculator className="w-5 h-5 md:w-8 md:h-8" />
            </div>
            <div className="flex flex-col text-left leading-none">
              <span className="font-black text-lg md:text-3xl tracking-tighter uppercase leading-none ">Proyect<span className="text-sky-400">AR</span></span>
              {/* MODIFICADO: Versión en el Nav */}
              <span className="text-[9px] md:text-[11px] font-black tracking-[0.2em] text-slate-500 uppercase mt-1 md:mt-3 opacity-60 leading-none">v1.1 (PRO)</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 md:gap-10 w-full md:w-auto justify-between md:justify-end">
            ...
            <button onClick={() => setDarkMode(!darkMode)} className="p-2.5 md:p-4 rounded-2xl bg-slate-100 dark:bg-slate-800 border dark:border-slate-700 shadow-md active:scale-90">
        ...
        <footer className="max-w-[1800px] mx-auto w-full border-t dark:border-slate-800 mt-10 md:mt-20 py-10 md:py-16 px-6 md:px-10 flex flex-col lg:flex-row justify-between items-center gap-10">
          <div className="flex-1 text-center lg:text-left leading-none">
          ...
          <div className="flex-1 flex flex-col items-center lg:items-end gap-2 text-[11px] font-bold text-slate-400 uppercase opacity-50 italic">
            <a href="https://github.com/MaxiNavarro97" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:text-indigo-400 transition-colors leading-none"><Github className="w-4 h-4" /> @MaxiNavarro97</a>
            <p className="leading-none">@maxinavarro1997@gmail.com</p>
          </div>
        </footer>
      </div>
    </div>
